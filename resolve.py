...
def resolve(row, param, gauge_dfs, view_width=int(28*2), save=False, saveloc=None):
    ...
    mean = round(row['Pre-effect Mean'],2)
    stddev = round(row['Pre-effect Stddev'],2)
    ...
    if not np.isnan(n):
        ...
        ax1.axhline(mean, linestyle='dashed', color='blue')
...
for param, df in param_dfs.items():
    ...
    for i,row in enumerate(df.iterrows()):
        ...
        resolve(row, param, station_dfs, view_width=int(28*3), save=True, saveloc=os.path.join(plot_folder,param))
