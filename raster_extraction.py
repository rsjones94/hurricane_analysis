...
df = unpack_timeseries(gauges.gauge,dates,rows)
...
new = pd.merge(sta,r,left_index=True,right_index=True)
"""
...
for gauge_no, gauge_df in station_dfs.items():
    ...
    try:
        rain = df[gauge_no]
        rain.name = 'Rain'
        new = pd.merge(gauge_df, rain, left_index=True, right_index=True)
...
for gauge_no, gauge_df in station_dfs.items():
    print(f'Saving {gauge_no}')
    gauge_df.to_csv(os.path.join(stations_parent,gauge_no+'.csv'))
